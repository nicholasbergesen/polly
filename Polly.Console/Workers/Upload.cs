        ...
        protected override async Task DoWorkInternalAsync(CancellationToken token)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                .ContinueWith(ctask =>
                {
                    ...
                    System.Console.WriteLine();
                    System.Console.WriteLine($"Runnuing tasks remaining:{tasks.Select(x => !x.IsCompleted).Count()}{Environment.NewLine}");
                    System.Console.WriteLine($"A task ended unexpectedly:{ctask.Exception.ToString()}");
